...
void FormatMC1(TH1 * mc1hist){
  ...
  mc1hist->SetMarkerColor(3);
  mc1hist->SetFillColor(3);
  mc1hist->SetLineColor(3);
  //mc1hist->SetFillColor(kRed);
  //mc1hist->SetLineColor(kRed);
...
// Write "BESIII" in the upper right corner
void WriteBelle(){
  TLatex * bes3 = new TLatex(0.94,0.94, "BELLE");
  bes3->SetNDC();
  bes3->SetTextFont(72);
  bes3->SetTextSize(0.1);
  bes3->SetTextAlign(33);
...
void MakeLegend(TH1 * datahist,   // Histogram with data
  ...
  if(datahist && dataname)
    leg->AddEntry(datahist, dataname, "EP");
  if(mc1hist && mc1name)
    leg->AddEntry(mc1hist, mc1name, "LF");
  if(mc2hist && mc2name)
...
// Plot a data MC plot
void PlotDataMC2(char * filename,  // Name for the output files, 
                                  // without extension 
                TH1 * datahist,   // Histogram with data
                char * dataname,  // Description of data
                TH1 * mc1hist, // Histogram with first MC
  ...
  TCanvas * c1 = new TCanvas("belleplots","BELLE Plots", 800,600);
