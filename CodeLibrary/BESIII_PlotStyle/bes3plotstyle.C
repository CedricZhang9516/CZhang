...
void PlotDataMC(char * filename,  // Name for the output files, 
  ...
  sprintf(filenameall,"%s", filename);

    TLatex *t = new TLatex(0.1,0.1,filenameall);
    t->Draw("same");
  }
...
void PlotDataMC2(char * filename,  // Name for the output files, 
  ...
  TCanvas * c1 = new TCanvas("belleplots","BESIII Plots", 800,600);
  ...
  //NameAxes(mc1hist,filenameall,filenameall);
  //c1->SetTitle(filenameall);
  mc1hist->Draw("axis");
  //datahist->Draw("axis");
  if(mc2hist)
    mc2hist->Draw("histsame");
