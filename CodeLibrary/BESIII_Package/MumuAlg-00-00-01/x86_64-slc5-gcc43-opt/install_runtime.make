...
jobOptions_Mumu_txt_dependencies = ../share/jobOptions_Mumu.txt


install_runtimeinstall :: ${install_dir}/jobOptions_Mumu.txt
	@if test ! "${installarea}" = ""; then\
	  echo "installation done"; \
	fi

${install_dir}/jobOptions_Mumu.txt :: ../share/jobOptions_Mumu.txt
	@if test ! "${installarea}" = ""; then \
	  d=`dirname ../share/jobOptions_Mumu.txt`; \
	  d=`(cd $${d}; pwd)`; \
	  CMTINSTALLAREA=${CMTINSTALLAREA}; export CMTINSTALLAREA; \
	  $(cmt_install_action) "$${d}" "jobOptions_Mumu.txt" "$(install_dir)" "cp -R" ; \
	fi
...
install_runtimeuninstall ::
	@if test ! "${installarea}" = ""; then \
	  ...
	  d=`(cd $${d}; pwd)`; \
	  CMTINSTALLAREA=${CMTINSTALLAREA}; export CMTINSTALLAREA; \
	  $(cmt_uninstall_action) "$${d}" "jobOptions_Mumu.txt" "$(install_dir)" "cp -R" ; \
	fi
...
#	@echo "#CMT> Warning: $@: No rule for such target" >&2; exit
	if echo $@ | grep '$(package)setup\.make$$' >/dev/null; then\
	 echo "$(CMTMSGPREFIX)" "(install_runtime.make): $@: File no longer generated" >&2; exit 0; fi
else
.DEFAULT::
	$(echo) "(install_runtime.make) PEDANTIC: $@: No rule for such target" >&2
	if echo $@ | grep '$(package)setup\.make$$' >/dev/null; then\
	 echo "$(CMTMSGPREFIX)" "(install_runtime.make): $@: File no longer generated" >&2; exit 0;\
