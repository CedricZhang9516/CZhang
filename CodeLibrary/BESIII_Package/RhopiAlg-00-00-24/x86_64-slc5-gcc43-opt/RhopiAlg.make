...
#  Library RhopiAlg
#
#   Generated Wed Oct 28 00:12:00 2015  by zhangc
#
#====================================
...
ifndef QUICK
$(bin)RhopiAlg_dependencies.make : $(src)Rhopi.cxx $(src)components/Rhopi_load.cxx $(src)components/Rhopi_entries.cxx $(use_requirements) $(cmt_final_setup_RhopiAlg)
	$(echo) "(RhopiAlg.make) Rebuilding $@"; \
	  $(build_dependencies) RhopiAlg -all_sources -out=$@ $(src)Rhopi.cxx $(src)components/Rhopi_load.cxx $(src)components/Rhopi_entries.cxx
endif
...
$(bin)$(binobj)Rhopi.d : $(use_requirements) $(cmt_final_setup_RhopiAlg)

$(bin)$(binobj)Rhopi.d : $(src)Rhopi.cxx
	$(dep_echo) $@
	$(cpp_silent) $(cppcomp) $(cppdepflags) -o $(@D)/Rhopi.dep $(use_pp_cppflags) $(RhopiAlg_pp_cppflags) $(lib_RhopiAlg_pp_cppflags) $(Rhopi_pp_cppflags) $(use_cppflags) $(RhopiAlg_cppflags) $(lib_RhopiAlg_cppflags) $(Rhopi_cppflags) $(Rhopi_cxx_cppflags)  $(src)Rhopi.cxx
	$(cpp_silent) $(format_dependencies) $@ $(@D)/Rhopi.o $(src)Rhopi.cxx $(@D)/Rhopi.dep
endif
endif
...
$(bin)$(binobj)Rhopi_load.d : $(use_requirements) $(cmt_final_setup_RhopiAlg)

$(bin)$(binobj)Rhopi_load.d : $(src)components/Rhopi_load.cxx
	$(dep_echo) $@
	$(cpp_silent) $(cppcomp) $(cppdepflags) -o $(@D)/Rhopi_load.dep $(use_pp_cppflags) $(RhopiAlg_pp_cppflags) $(lib_RhopiAlg_pp_cppflags) $(Rhopi_load_pp_cppflags) $(use_cppflags) $(RhopiAlg_cppflags) $(lib_RhopiAlg_cppflags) $(Rhopi_load_cppflags) $(Rhopi_load_cxx_cppflags) -I../src/components $(src)components/Rhopi_load.cxx
	$(cpp_silent) $(format_dependencies) $@ $(@D)/Rhopi_load.o $(src)components/Rhopi_load.cxx $(@D)/Rhopi_load.dep
endif
endif
...
$(bin)$(binobj)Rhopi_entries.d : $(use_requirements) $(cmt_final_setup_RhopiAlg)

$(bin)$(binobj)Rhopi_entries.d : $(src)components/Rhopi_entries.cxx
	$(dep_echo) $@
	$(cpp_silent) $(cppcomp) $(cppdepflags) -o $(@D)/Rhopi_entries.dep $(use_pp_cppflags) $(RhopiAlg_pp_cppflags) $(lib_RhopiAlg_pp_cppflags) $(Rhopi_entries_pp_cppflags) $(use_cppflags) $(RhopiAlg_cppflags) $(lib_RhopiAlg_cppflags) $(Rhopi_entries_cppflags) $(Rhopi_entries_cxx_cppflags) -I../src/components $(src)components/Rhopi_entries.cxx
	$(cpp_silent) $(format_dependencies) $@ $(@D)/Rhopi_entries.o $(src)components/Rhopi_entries.cxx $(@D)/Rhopi_entries.dep
endif
endif
...
#	@echo "#CMT> Warning: $@: No rule for such target" >&2; exit
	if echo $@ | grep '$(package)setup\.make$$' >/dev/null; then\
	 echo "$(CMTMSGPREFIX)" "(RhopiAlg.make): $@: File no longer generated" >&2; exit 0; fi
else
.DEFAULT::
	$(echo) "(RhopiAlg.make) PEDANTIC: $@: No rule for such target" >&2
	if echo $@ | grep '$(package)setup\.make$$' >/dev/null; then\
	 echo "$(CMTMSGPREFIX)" "(RhopiAlg.make): $@: File no longer generated" >&2; exit 0;\
