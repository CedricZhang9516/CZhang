...
TH2D * newTH2D(Char_t name[]="h2",TH1 *hx = 0,TH1 *hy = 0){
  if(hx == NULL){
    printf("TH2D %s not created since hx==NULL\n",name);
    return NULL;
  }
  if(hy == NULL){
    printf("TH2D %s not created since hy==NULL\n",name);
    return NULL;
  }
  ...
  TH2D *h2 = new TH2D(name,"",nbinx,xLow,xHigh,nbiny,yLow,yHigh);
  h2->SetTitle(Form(";%s;%s",hx->GetName(),hy->GetName()));
  return h2;
}
...
void txtA(Double_t x0=0.3, Double_t y0=0.3,TString txts="",Int_t iColor=2,Double_t fSize=0.05,Double_t fang=0.0,Bool_t IsNDC=kTRUE){
  ...
  if(!(x0>=0 && x0<=1 && y0>=0 && y0<=1) ) ltx->SetNDC(kFALSE);//若是绝对位置坐标，那就不用相对的
  ltx->SetTextColor(iColor);
  ltx -> SetTextFont(22);
  ltx->SetTextSize(fSize);

  ltx->SetTextAngle(fang);//有角度的字
  ltx->DrawLatex(x0,y0,txts.Data());
}
...
void SetMode(TGraph *grTmp,Int_t iMode=0){
    Int_t iMarker[] = {20,21,24,25,28,29,30,27,3,  5,2, 26,22,23};
    Int_t iColor[]  = { 2, 4, 6, 1, 9,50,40,31,41,35,44,38,47,12};
    grTmp ->SetMarkerStyle(iMarker[iMode]);
    grTmp ->SetMarkerColor(iColor[iMode]);
...
void txtMY(Double_t x0, Double_t y0,TH1 *h,Char_t sFormat[]="My=%.2f",Double_t fsize = 0.06){
  h->SetStats(kFALSE);
  TLatex *ltx = new TLatex();
  ltx->SetNDC(kTRUE);
  ltx->SetTextColor(h->GetLineColor());
...
TLorentzVector boostToMotherFrame(TLorentzVector DauToBeBoost,TLorentzVector m){
  TVector3 bm = m.BoostVector();
  TLorentzVector newDau = DauToBeBoost;//子粒子，赋值
  newDau.Boost(-bm);
  return newDau;
}
//从root读出his
TH1 *readTHinROOT(Char_t rootName[],Char_t histName[]){
  ...
  fin.Close();//即使文件关闭，因为有setdirectory，指针就不会空了。
  if(htmpTH1F==NULL){
...
void MakeLegend(TH1 * datahist,   // Histogram with data
  ...
  TLegend * leg = new TLegend(xlow, ylow, xhi, yhi);
  if(datahist && dataname)
    leg->AddEntry(datahist, dataname, datahist->GetDrawOption());
  if(mc1hist && mc1name)
    leg->AddEntry(mc1hist, mc1name, mc1hist->GetDrawOption());
...
//排序：
//根据第一个数组的序列（整形）为索引来进行排序（浮点数）1，2，3，4->123,23,...
...
//要填充文本文件
Double_t readTxt2TH1(Char_t filename[],TH1 *h,Bool_t clearH=kTRUE){
  if(clearH) h->Clear();
  FILE *pf = fopen(filename,"rt");
  Int_t n=1;
  Double_t d;
...
void ShowContent(TH1 *h, Int_t itxtColor=kRed, Double_t txtSize=0.07, Double_t fangle=45,Bool_t NoZeroBin=kTRUE){
   for(Int_t i=0; i<h->GetNbinsX();i++){
     Double_t x0 = h->GetBinCenter(i+1);//每个bin中间的那个数
     Double_t y0 = h->GetBinContent(i+1);//注意第一个Bin的编号是1，第0个Bin填充underflow，也就是小于范围的,而bin(n+1)则保存overflow
...
void ShowContent(TGraph *gr, Int_t itxtColor=kRed, Double_t txtSize=0.07, Double_t fangle=45,Bool_t NoZeroBin=kTRUE){
   for(Int_t i=0; i<gr->GetN();i++){//getN 拿出有多少数据点
     Double_t x0,y0; 
     gr->GetPoint(i,x0,y0);//取出XY值
...
Char_t *OutValG(Double_t Val,Double_t Err,Double_t factor=1){
  Val= Val*factor;
  Err=Err*factor;
  Int_t ndig = getNumDigsFromErr(Err);

  Char_t sFormat[100];
  sprintf(sFormat,"%%.%dg#pm%%.%dg",ndig,ndig);
...
Double_t Mom2Evis_Eplus(Double_t Pin){
  Double_t mass = 5.10998909999999971e-1;
  TLorentzVector mom;
  mom.SetXYZM(0,0,Pin,mass);
  Double_t Evis = mom.E()+mass;
  return Evis;
}

Double_t Mom2Evis_IBD(Double_t Pin){
  return Pin-0.782;
}


void FitGausOnH1(TH1F *h,Double_t *Mean,Double_t *Sigma, Double_t *MeanE, Double_t *SigmaE){
  Int_t maxBin = h->GetMaximumBin();
  Double_t fmax = h->GetBinContent(maxBin);
  Double_t fMean0 = h->GetBinCenter(maxBin);
  Double_t thresh = fmax*0.001;
  //find to left range..
  Int_t iL = maxBin-1;
  Int_t iN0 = 0;
  for(Int_t i=maxBin-1; i>1; i--){
    Double_t d = h->GetBinContent(i);
    if(d > thresh){
      iL = i;
      iN0 = 0;
    }else {
...
/*   TCanvas *c1 = new TCanvas("c1",""); */
/*   h->Draw(); */
  Double_t fL = h->GetBinCenter(iL);
  Double_t fR = h->GetBinCenter(iR);

  TF1 *fun = new TF1("fun","[0]*TMath::Gaus(x,[1],[2])",fL,fR);
  fun->SetParameter(0,1);
  fun->SetParameter(1,fMean0);
  fun->SetParameter(2,(fR-fL)/6.0);
  Double_t PeakVal = fun->Eval(fMean0);
  fun->SetParameter(0,fmax*PeakVal);

  h->Fit("fun","RL");
  //  c1->SaveAs("tmp.root");
  *Mean = fun->GetParameter(1);
  *Sigma =TMath::Abs( fun->GetParameter(2));
  *MeanE = fun->GetParError(1);
  *SigmaE = fun->GetParError(2);
  delete fun;
...
TGraphErrors *ReadGraphErrors(TString rootFile, TString grName){
  TFile fin(Form("%s",rootFile.Data()),"read");
  return (TGraphErrors *)fin.Get(Form("%s",grName.Data()));
}
#endif 
